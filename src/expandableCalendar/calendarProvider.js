...
class CalendarProvider extends Component {
  ...
  isPastDate(date) {
    //moment.utc(moment.tz(userInfo.TimeZone)).format('MMM YYYY')
    const today = moment.utc(moment.tz(this.props.tz))
    const d = moment(date)
    const isWhat = moment(today).diff(moment(d));


    console.log({isWhat,today,d})
    if(isWhat <0){
      return true
    }
  ...
  onTodayPress = () => {
    ...
    console.log({ttDay})
    this.setDate(ttDay, UPDATE_SOURCES.TODAY_PRESS);
