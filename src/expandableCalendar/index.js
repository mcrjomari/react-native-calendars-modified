...
const SPEED = 20;
const BOUNCINESS = isLowPhone?1: 6;
const CLOSED_HEIGHT = 125; // header + 1 week
const WEEK_HEIGHT = 46;
const KNOB_CONTAINER_HEIGHT = isLowPhone? 15:20;
...
class ExpandableCalendar extends Component {
  ...
  onPressArrowLeft = () => {
    const {getNow,setNow,setSelDate, reloadToday} = this.props
    if (this.state.position === POSITIONS.OPEN) {
      this.scrollPage(false,'left');
    }else{
      let newMonth = getNow;
      const fromWhat = moment(getNow.monthStartDate)
        .subtract(1, 'months')
        .format('YYYY-MM-DD');
      const lastFwhat = fromWhat.split('-');
      const lst = `${lastFwhat[0]}-${lastFwhat[1]}-01`;

      const toWhat = moment(getNow.monthEndDate)
        .subtract(1, 'months')
        .clone()
        .endOf('month')
        .format('YYYY-MM-DD');
  ...
  onPressArrowRight = () => {
    const {getNow,setNow,setSelDate} = this.props
    if (this.state.position === POSITIONS.OPEN) {
      this.scrollPage(true,'right');
    }else{
...
//     console.log(_.first(value),'bbs')
//     if(_.first(value) == undefined){

// //       const newDt = moment(new Date()).utc(moment.tz(this.props.tz)).format('YYYY-MM-DD')
// //       setTimeout(()=>{
// //         this.props.setSelDate(newDt)
// //       },500)

// //       //this.props.calProv.current.onTodayPress()
// //       const aa = newDt.split('-')[0]
// //       const bb =newDt.split('-')[1]
// //       const cc = newDt.split('-')[2]
// //       const newFirstB = Number(bb)
// //       const newFirstA = Number(aa)
// //       const newFirstC = Number(cc)

// //       const newTT = moment(newDt).valueOf()
     
// //       //this.visibleMonth = newFirstB
// //       const newF = {
// //         dateString:newDt,
// //         day: newFirstC,
// //         month: newFirstB,
...
// // //
// //       if (this.visibleMonth !== this.getMonth(newDt) && updateSource !== UPDATE_SOURCES.DAY_PRESS) {
// //         const next = this.isLaterDate(newF, newDt);
// //        // this.scrollPage(next);
        
// //         const d = parseDate(newDt);
...
//       // const next = this.isLaterDate(newF, this.props.getNow.monthStartDate);
//       // console.log('dito1',{next})
//       // this.scrollPage(true);

//       // updating openHeight
//       setTimeout(() => {
//         // to wait for setDate() call in horizontal scroll (this.scrollPage())
//         const numberOfWeeks = this.getNumberOfWeeksInMonth(parseDate(newDt));
...
//     }else 
  ...
  renderWeekCalendar() {
    const {position,setSelDate} = this.state;
  ...
  renderKnob() {
    ...
    return (
      <View style={this.style.knobContainer} pointerEvents={'none'} testID={`${this.props.testID}-knob`}>
        ...
        {/* <View  onPress={()=>{
          this.setState({position:this.state.position === 'open'?'closed':'open'})
        }}  >
        {this.state.position === 'open'?(
          <Icon name={'chevron-up'} size={30} color={Theme.Gray6} />
        ):(
          <Icon name={'chevron-down'} size={30} color={Theme.Gray6} />
        )}
        </View> */}
        {/* <Icon name={'chevron-up'} size={30} color={Theme.Gray6} /> */}
      </View>
  ...
  renderArrow = direction => {
    ...
    return (
      ...
      <Icon name={direction === 'right' ? 'chevron-right':'chevron-left'} style={this.style.arrowImage} size={25} color={Theme.Gray6} testID={`${this.props.testID}-${direction}-arrow`} />
    );
  };

  render() {
    const {setListLoading,chkProv,selDate,setCurrMM,context,setArrangeData,tz,setSelDate,getNow,reloadToday,setNow,getNowOrig,currDD, isTodayActive,calProv,style, hideKnob, horizontal, allowShadow, theme, ...others} = this.props;
          ...
          >
            <CalendarList
             ...
             onVisibleMonthsChange={this.onVisibleMonthsChange}
              ...
              setArrowClicked={this.props.setArrowClicked}
