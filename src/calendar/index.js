...
class Calendar extends Component {
  ...
  _handleDayInteraction(date, interaction) {
    ...
    const minDate = parseDate(this.props.getNow.minMin);
    const maxDate = parseDate(this.props.getNow.maxMax);
  ...
  getState(day) {
    const {disabledByDefault,tz,getNow} = this.props;
   
    
    const minDate = parseDate(getNow.minMin);
    const maxDate = parseDate(getNow.maxMax);
    ...
    const ttDay = moment(new Date()).utc(moment.tz(tz)).format('YYYY-MM-DD')
    const vDate = day.toString('yyyy-MM-dd');
    const nDay = moment(vDate).format('YYYY-MM-DD')

    const sameMe = ttDay === nDay
    ...
    return (
      <View style={this.style.dayContainer} key={`week-container-${weekNumber}`}>
        <BasicDay
          ...
          setSelDate={this.props.setSelDate}
        >
          {weekNumber}
        </BasicDay>
      </View>
  ...
  renderDay(day, id) {
    ...
    const {hideExtraDays,setSelDate} = this.props;
